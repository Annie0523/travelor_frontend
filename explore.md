...
<div id="filter-section">
  ...
  <label class="wonder-checkbox">
      <input type="checkbox" class="wonder-option" value="sao_paulo" checked/> São Paulo
    </label>
    <label class="wonder-checkbox">
...
<div id="filter-section">
    ...
    <input type="text" id="interest-filter" placeholder="e.g. shopping, nightlife..."/>
</div>
...
// Create markers for the The cities  (blue pin)
...
// Helper function: generate an inline SVG data URI (a colored pin),
...
// Place / Move the user-selected marker (red pin)
...
// Apply filters based on what the user wants to input
...
wondersMarkers.forEach(({ wonder, marker }) => {
    let isVisible = true;
    // Check wonder checkbox
    if (!activeWonders.includes(wonder.value)) {
      isVisible = false;
    }
    // Category filter
